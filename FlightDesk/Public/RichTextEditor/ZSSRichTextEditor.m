...
@interface ZSSRichTextEditor ()<PhotoTweaksViewControllerDelegate>
{
    UIPopoverController *fontPopoverController;
...
- (void)viewDidLoad {
    ...
    [[AppDelegate sharedDelegate].window.rootViewController.view addSubview:self.toolbarHolder];
...
- (void)createToolBarScroll {
    ...
    self.toolBarScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, [self isIpad] ? [UIScreen mainScreen].bounds.size.width : [UIScreen mainScreen].bounds.size.width - 44, 44)];
...
- (void)createParentHoldingView {
    ...
    //Background Toolbar
    UIToolbar *backgroundToolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 44)];
    ...
    if (_alwaysShowToolbar) {
        self.toolbarHolder.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, 44);
    } else {
        self.toolbarHolder.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, 44);
    }
...
- (void)setPlaceholderText {
    ...
    if (self.placeholder != NULL && [self.placeholder length] != 0) {
        NSString *js = [NSString stringWithFormat:@"zss_editor.setPlaceholder(\"%@\", 1);", self.placeholder];
        if (![[AppDelegate sharedDelegate].userLevel.lowercaseString isEqualToString:@"admin"]) {
            js = [NSString stringWithFormat:@"zss_editor.setPlaceholder(\"%@\", 0);", self.placeholder];
        }
        [self.editorView stringByEvaluatingJavaScriptFromString:js];
...
- (void)showFontsPicker:(UIBarButtonItem*)buttonItem {
    ...
    fontPopoverController= [[UIPopoverController alloc] initWithContentViewController:fontPicker];
    [fontPopoverController setPopoverContentSize:CGSizeMake(220,400) animated:NO];
    [fontPopoverController presentPopoverFromRect:[view bounds] inView:view permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
}

- (void)setSelectedFontFamily:(ZSSFontFamily)fontFamily {
    
    [fontPopoverController dismissPopoverAnimated:YES];
...
{
    CGRect rectToCalScale = self.view.frame;
    float scale =  (rectToCalScale.size.width - 150) / croppedImage.size.width;
    //Scale the image
    CGSize targetSize = CGSizeMake(croppedImage.size.width * scale, croppedImage.size.height * scale);
...
- (void)keyboardWillShowOrHide:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            frame.origin.y = [UIScreen mainScreen].bounds.size.height - (keyboardHeight + sizeOfToolbar);
            self.toolbarHolder.frame = frame;
            
            // Editor View
            CGRect editorFrame = self.editorView.frame;
            editorFrame.size.height = (self.view.frame.size.height - keyboardHeight) - sizeOfToolbar - extraHeight + 70;
            ...
            sourceFrame.size.height = (self.view.frame.size.height - keyboardHeight) - sizeOfToolbar - extraHeight + 70;
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            if (_alwaysShowToolbar) {
                frame.origin.y = [UIScreen mainScreen].bounds.size.height - sizeOfToolbar;
            } else {
                frame.origin.y = [UIScreen mainScreen].bounds.size.height + keyboardHeight;
            }
