...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0)
    {
        ...
        CGRect rectOfHeader = cell.frame;
        rectOfHeader.size.width = self.frame.size.width;
        [cell setFrame:rectOfHeader];
        cell.date = data.date;
    ...
    if (data.isGeneralBanner == YES) {
        for (UIView *elementsView in data.view.subviews) {
            if ([elementsView isKindOfClass:[UIImageView class]]) {
                UIImageView *tmpImageView = (UIImageView *)elementsView;
                [elementsView removeFromSuperview];
                CGRect imageRect = elementsView.frame;
                [tmpImageView setFrame:CGRectMake(imageRect.origin.x, imageRect.origin.y, self.bounds.size.width, data.view.frame.size.height)];
                [data.view addSubview:tmpImageView];
                break;
            }
        }
    }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView_
{
    ...
    [self.bubbleTblDelegate scrollViewDidScrollWithBubble:self ScrView:scrollView_];
