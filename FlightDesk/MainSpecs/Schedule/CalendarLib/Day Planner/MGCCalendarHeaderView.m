...
- (NSArray*)weekDaysFromDate:(NSDate*)date
{
    ...
    NSDateComponents* components = [calendar2 components:(NSCalendarUnitYearForWeekOfYear | NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekOfYear | NSCalendarUnitWeekday | NSCalendarUnitHour | NSCalendarUnitMinute) fromDate:date];
    ...
    for (int i = 1; i <= 7; i++) { //1 is the comopnent for the first day of week 7 the last
        ...
        NSDate* date = [calendar2 dateFromComponents:components];
...
- (void)selectDate:(NSDate *)date fromTap:(BOOL)isTapped{
    NSLog(@"current Date :%@",  [self.detailsDateFormater stringFromDate:date]);
    NSLog(@"selected Date :%@",  [self.detailsDateFormater stringFromDate:self.selectedDate]);
    ...
    if(![self.calendar isDate:date inSameDayAsDate:self.selectedDate]){
        ...
        [AppDelegate sharedDelegate].isUpdatedMiniMonthBySelection = YES;
