...
- (void)getPreEventsFromLocal{
    ...
    if (fetchedResourcesCalendars == nil) {
        ...
        if (resourcesCalendar.alertTimeInterVal != nil) {
            alertVal = [resourcesCalendar.alertTimeInterVal integerValue];
        }else{
            alertVal = -1;
        }
        ...
        for (Users *userInfo in arrayUsersCalendarsSelected) {
            ...
            for (NSMutableDictionary *dict in preEventsFromLocalIdentify) {
                ...
                if ([calendarName isEqualToString:[NSString stringWithFormat:@"FD-U-(%@ %@ %@)", userInfo.firstName, userInfo.middleName, userInfo.lastName]]) {
                    ...
                    if (alertVal >= 0) {
                        [[AppDelegate sharedDelegate] updateLocalNotificationWithReservation:txtTitle.text withTimeInterVal:alertVal withAlertTitle:txtTitle.text withStartDate:startDate withEventIdentify:[dict objectForKey:@"identify"]];
                    }else{
            ...
            if (!isExit) {
                ...
                if (alertVal >= 0) {
                    [[AppDelegate sharedDelegate] updateLocalNotificationWithReservation:txtTitle.text withTimeInterVal:alertVal withAlertTitle:txtTitle.text withStartDate:startDate withEventIdentify:ev.eventIdentifier];
                }
            }
        }
...
- (void)updateCurrentEventInLocal:(EKEvent *)event withLocalID:(NSNumber *)eventLocalId{
    ...
    if (fetchedResourcesCalendars == nil) {
        ...
        resourcesCalendar.aircraft = [self getAircraftGroupName];
        resourcesCalendar.classroom = [self getClassroomsGroupName];
...
- (NSString *)getClassroomsGroupName{
    NSString *classroomGroupName = @"";
    for (NSString *classroomName in arrayClassroomsCalendarsSelected) {
        if ([classroomGroupName isEqualToString:@""]) {
            classroomGroupName = classroomName;
        }else{
            classroomGroupName = [NSString stringWithFormat:@"%@, %@", classroomGroupName, classroomName];
        }
        
    }
    return classroomGroupName;
}

- (NSString *)getAircraftGroupName{
    NSString *aircraftsNames = @"";
    for (Aircraft *aircraft in arrayAircraftsCalendarsSelected) {
        NSString *aircraftItems = aircraft.aircraftItems;
        NSData *data = [aircraftItems dataUsingEncoding:NSUTF8StringEncoding];
        NSArray *json = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        ...
        if ([aircraftsNames isEqualToString:@""]) {
            aircraftsNames = [NSString stringWithFormat:@"%@ %@", aircraftReg, aircraftMod];
        }else{
            aircraftsNames = [NSString stringWithFormat:@"%@, %@ %@", aircraftsNames, aircraftReg, aircraftMod];
        }
...
- (void)didChangeDate:(DateFlightCell *)_cell withDate:(NSDate *)_date{
    ...
    for (Users *userInfo in usersArray) {
        NSString *resourceUser = [NSString stringWithFormat:@"%@ %@ %@",userInfo.firstName,userInfo.middleName,userInfo.lastName];
        NSString *calendarName = [NSString stringWithFormat:@"FD-U-(%@)",resourceUser];
                ...
                if ([arrayUsersCalendarsSelected containsObject:userInfo]) {
                    ALAlertBanner *banner = [ALAlertBanner alertBannerForView:self.view
                                                                     ...
                                                                     position:ALAlertBannerPositionTop
                                                                        title:[NSString stringWithFormat:@"%@ was already booked!",resourceUser]];
                    [banner show];
                    //[AFMInfoBanner showWithText:[NSString stringWithFormat:@"%@ was already booked!",resourceUser] style:AFMInfoBannerStyleError andHideAfter:1.0];
    ...
    for (Aircraft *aircraft in aircraftArray) {
        ...
        NSString *resourceAircraft = [NSString stringWithFormat:@"%@ %@", aircraftReg, aircraftMod];
        NSString *calendarName = [NSString stringWithFormat:@"FD-A-(%@)", resourceAircraft];
            ...
            if (![self checkReservationsWithDate:calendarName]) {
                if ([arrayAircraftsCalendarsSelected containsObject:aircraft]) {
                    ALAlertBanner *banner = [ALAlertBanner alertBannerForView:self.view
                                                                        style:ALAlertBannerStyleNotify
                                                                     position:ALAlertBannerPositionTop
                                                                        title:[NSString stringWithFormat:@"%@ was already booked!",resourceAircraft]];
                    [banner show];
                    //[AFMInfoBanner showWithText:[NSString stringWithFormat:@"%@ was already booked!",resourceAircraft] style:AFMInfoBannerStyleError andHideAfter:1.0];
    ...
    for (NSString *classroomName in classroomsArray) {
        NSString *calendarName = [NSString stringWithFormat:@"FD-C-(%@)",classroomName];
            ...
            if (![self checkReservationsWithDate:calendarName]) {
                if ([arrayClassroomsCalendarsSelected containsObject:classroomName]) {
                    ALAlertBanner *banner = [ALAlertBanner alertBannerForView:self.view
                                                                        style:ALAlertBannerStyleNotify
                                                                     position:ALAlertBannerPositionTop
                                                                        title:[NSString stringWithFormat:@"%@ was already booked!",classroomName]];
                    [banner show];
                    //[AFMInfoBanner showWithText:[NSString stringWithFormat:@"%@ was already booked!",classroomName] style:AFMInfoBannerStyleError andHideAfter:2.0];
         ...
         completion:^(BOOL finished)
         {
             [AddReservationTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:indexPath.section]
                                   atScrollPosition:UITableViewScrollPositionBottom
                                           animated:YES];
             if (indexPath.row == 0) {
                 [AddReservationTableView collapseCellAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:indexPath.section]];
             }else{
                 [AddReservationTableView collapseCellAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:indexPath.section]];
             }
         }
         ];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    if (textField == txtTitle) {
        NSArray *titleComponents = [txtTitle.text componentsSeparatedByString:@" by "];
        if (titleComponents.count > 1) {
            txtTitle.text = titleComponents[0];
        }
    }
}
- (void)textFieldDidEndEditing:(UITextField *)textField{
    if (textField == txtTitle) {
        txtTitle.text = [NSString stringWithFormat:@"%@ by %@ %@ %@", txtTitle.text, [AppDelegate sharedDelegate].clientFirstName, [AppDelegate sharedDelegate].clientMiddleName, [AppDelegate sharedDelegate].clientLastName];
    }
}
