...
@interface ScheduleMainViewController ()<YearViewControllerDelegate, WeekViewControllerDelegate, DayViewControllerDelegate,MonthViewControllerDelegate, UIScrollViewDelegate,UIPopoverPresentationControllerDelegate, AddReservationViewControllerDelegate, MGCMonthMiniCalendarViewDelegate>
{
...
//    }
    ...
    foucesDate = [self.dateFormatter dateFromString:@"2000-01-01"];
    _calendarView = [[MGCMonthMiniCalendarView alloc]initWithFrame:miniMonthView.frame];
    ...
    _calendarView.daysFont = [UIFont systemFontOfSize:12];
    [self updatingMiniMonthView:[NSDate date]];
    [miniMonthView addSubview:_calendarView];
}
- (NSAttributedString*)headerTextForMonthAtIndexPath:(NSDate *)date
{
    self.dateFormatter.dateFormat = @"MMMM yyy";
    NSString *dateStr = [[self.dateFormatter stringFromDate:date]uppercaseString];
    return [[NSAttributedString alloc]initWithString:dateStr attributes:@{ NSFontAttributeName:[UIFont boldSystemFontOfSize:17]}];
}
- (void)updatingMiniMonthView:(NSDate *)date{
    if ([self.calendar mgc_isDate:foucesDate sameDayAsDate:date]){
        return;
    }
    if ([self.calendar mgc_isDate:foucesDate sameMonthAsDate:date]) {
        //return;
    }
    foucesDate = date;
    _calendarView.date = date;
    _calendarView.headerText = [self headerTextForMonthAtIndexPath:[_calendar mgc_startOfMonthForDate:date]];
    
    _calendarView.highlightedDays = nil;
    if ([self.calendar mgc_isDate:date sameMonthAsDate:[NSDate date]])
    {
        NSUInteger i = [self.calendar components:NSCalendarUnitDay fromDate:[self.calendar mgc_startOfMonthForDate:date] toDate:[NSDate date] options:0].day + 1;
        _calendarView.highlightedDays = [NSIndexSet indexSetWithIndex:i];
        _calendarView.highlightColor = [UIColor redColor];
    }
    
    [_calendarView setNeedsDisplay];
    [AppDelegate sharedDelegate].isUpdatedMiniMonthBySelection = NO;
}
...
- (void)reloadEventsFromCalendarViewController{
    if ([self.calendarViewController isKindOfClass:[DayViewController class]]) {
        [(DayViewController *)self.calendarViewController reloadEvents];
    }else if ([self.calendarViewController isKindOfClass:[WeekViewController class]]) {
        [(WeekViewController *)self.calendarViewController reloadEvents];
    }
}
...
- (void)calendarViewController:(CalendarViewController*)controller didShowDate:(NSDate*)date
{
    ...
    [self updatingMiniMonthView:date];
}
...
- (void)redirectFromPush:(NSString *)eventIdenify{
    EKEvent *eventToUpdate = [self.eventStore eventWithIdentifier:eventIdenify];
    if (eventToUpdate) {
        if ([self.calendarViewController isKindOfClass:[DayViewController class]]) {
            
            [self.calendarViewController moveToDate:eventToUpdate.startDate animated:NO];
            
            AddReservationViewController *vc = [[AddReservationViewController alloc] init];
            vc.startDate = eventToUpdate.startDate;
            vc.endDate = eventToUpdate.endDate;
            vc.editEvent = eventToUpdate;
            vc.delegate = self;
            vc.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
            ...
            CalendarViewController *controllerNew = [self controllerForViewType:CalendarViewDayType];
            [self moveToNewController:controllerNew atDate:eventToUpdate.startDate];
            self.viewChooser.selectedSegmentIndex = CalendarViewDayType;
            self.view.userInteractionEnabled = NO;
            leftPaddingContainerCons.constant = 200.0f;
            [AppDelegate sharedDelegate].isSelectedDayViewForBooking = YES;
...
//}
- (void)selectedDateFromYearView:(NSDate *)selectedDate{
    [self.calendarViewController moveToDate:selectedDate animated:YES];
}
@end
