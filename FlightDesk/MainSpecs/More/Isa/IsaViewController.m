...
@interface IsaViewController ()<UITextFieldDelegate>
{
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"ISA%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"IsaViewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                UITextField *textField = (UITextField *)aView;
                textField.text=@"";
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"ISA%ld",(long)textField.tag+10000]];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"ISA%ld",(long)textField.tag+10000]];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }
        }
    }
...
- (IBAction)clearISA:(id)sender {
    ...
    ((UITextField *)[self.view viewWithTag:144]).text = @"";
    ((UITextField *)[self.view viewWithTag:145]).text = @"";
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcIsa" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcIsa" inManagedObjectContext:context];
            CalcIsa *calcIsa=[[CalcIsa alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcIsa.name=self.nameTextField.text;
            
            calcIsa.altElev = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:141]).text doubleValue]];
            calcIsa.baro = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:142]).text doubleValue]];
            calcIsa.temp = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:143]).text doubleValue]];
            calcIsa.diffC = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:144]).text doubleValue]];
            calcIsa.diffFT = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:145]).text doubleValue]];
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcIsa];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcIsa *calcIsaToDelete in fetchedData) {
            [context deleteObject:calcIsaToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
}
...
{
    static NSString *MyIdentifier = @"CalcIsaItem";
    ...
    CalcIsa *calcIsa=[arrayFiles objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcIsa *calcIsa=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcIsa];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcIsa*)calcIsa {
    ((UITextField *)[self.view viewWithTag:141]).text=[calcIsa.altElev doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcIsa.altElev doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:142]).text=[calcIsa.baro doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcIsa.baro doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:143]).text=[calcIsa.temp doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcIsa.temp doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:144]).text=[calcIsa.diffC doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcIsa.diffC doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:145]).text=[calcIsa.diffFT doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcIsa.diffFT doubleValue]]:@"";
