...
@interface CloudsViewController ()<UITextFieldDelegate>
{
...
@implementation CloudsViewController
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"CLO%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"CloudsviewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                UITextField *textField = (UITextField *)aView;
                textField.text=@"";
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"CLO%ld",(long)textField.tag+10000]];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"CLO%ld",(long)textField.tag+10000]];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }
        }
    }
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcClouds" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcClouds" inManagedObjectContext:context];
            CalcClouds *calcClouds=[[CalcClouds alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcClouds.name=self.nameTextField.text;
            
            calcClouds.cloudTemp = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:181]).text doubleValue]];
            calcClouds.dew = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:182]).text doubleValue]];
            calcClouds.bases = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:183]).text doubleValue]];
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcClouds];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcClouds *calcCloudsToDelete in fetchedData) {
            [context deleteObject:calcCloudsToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
...
{
    static NSString *MyIdentifier = @"CalcCloudsItem";
    ...
    CalcClouds *calcClouds=[arrayFiles objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcClouds *calcClouds=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcClouds];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcClouds*)calcClouds {
    ((UITextField *)[self.view viewWithTag:181]).text=[calcClouds.cloudTemp doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcClouds.cloudTemp doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:182]).text=[calcClouds.dew doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcClouds.dew doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:183]).text=[calcClouds.bases doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcClouds.bases doubleValue]]:@"";
