...
@interface SpeedViewController ()<UITextFieldDelegate>
{
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"SPD%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"SpeedViewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                UITextField *textField = (UITextField *)aView;
                textField.text=@"";
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"SPD%ld",(long)textField.tag+10000]];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"SPD%ld",(long)textField.tag+10000]];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }
        }
    }
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcSpeed" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcSpeed" inManagedObjectContext:context];
            CalcSpeed *calcSpeed=[[CalcSpeed alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcSpeed.name=self.nameTextField.text;
            
            calcSpeed.as = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:171]).text doubleValue]];
            calcSpeed.alt = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:172]).text doubleValue]];
            calcSpeed.tas = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:173]).text doubleValue]];
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcSpeed];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcSpeed *calcSpeedToDelete in fetchedData) {
            [context deleteObject:calcSpeedToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
}
...
{
    static NSString *MyIdentifier = @"CalcSpeedItem";
    ...
    CalcSpeed *calcSpeed=[arrayFiles objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcSpeed *calcSpeed=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcSpeed];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcSpeed*)calcSpeed {
    ((UITextField *)[self.view viewWithTag:171]).text=[calcSpeed.as doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcSpeed.as doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:172]).text=[calcSpeed.alt doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcSpeed.alt doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:173]).text=[calcSpeed.tas doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcSpeed.tas doubleValue]]:@"";
