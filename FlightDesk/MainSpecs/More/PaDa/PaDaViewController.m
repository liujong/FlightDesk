...
@interface PaDaViewController ()<UITextFieldDelegate>
{
...
@implementation PaDaViewController
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"PAD%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"PaDaViewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"PAD%ld",(long)textField.tag+10000]];
...
- (IBAction)clearPA:(id)sender {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",151+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",152+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",153+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",154+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
...
- (IBAction)clearDA:(id)sender {
    ...
    ((UITextField *)[self.view viewWithTag:165]).text = @"";
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",161+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",162+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",163+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",164+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"PAD%d",165+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
    [self.SavedRecordTableView reloadData];
}
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcPaDa" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcPaDa" inManagedObjectContext:context];
            CalcPaDa *calcPaDa=[[CalcPaDa alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcPaDa.name=self.nameTextField.text;
            
            calcPaDa.paAltElev = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:151]).text doubleValue]];
            calcPaDa.paRaro = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:152]).text doubleValue]];
            calcPaDa.paMilibars = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:153]).text doubleValue]];
            calcPaDa.pa = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:154]).text doubleValue]];
            
            calcPaDa.daAltElev = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:161]).text doubleValue]];
            calcPaDa.daTemp = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:162]).text doubleValue]];
            calcPaDa.daBaro = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:163]).text doubleValue]];
            calcPaDa.daDew = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:164]).text doubleValue]];
            calcPaDa.da = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:165]).text doubleValue]];
            
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcPaDa];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcPaDa *calcPaDaToDelete in fetchedData) {
            [context deleteObject:calcPaDaToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
}
...
{
    static NSString *MyIdentifier = @"CalcPaDaItem";
    ...
    CalcPaDa *calcPaDa=[arrayFiles objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcPaDa *calcPaDa=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcPaDa];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcPaDa*)calcPaDa {
    ((UITextField *)[self.view viewWithTag:151]).text=[calcPaDa.paAltElev doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.paAltElev doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:152]).text=[calcPaDa.paRaro doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.paRaro doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:153]).text=[calcPaDa.paMilibars doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.paMilibars doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:154]).text=[calcPaDa.pa doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.pa doubleValue]]:@"";
    
    ((UITextField *)[self.view viewWithTag:161]).text=[calcPaDa.daAltElev doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.daAltElev doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:162]).text=[calcPaDa.daTemp doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.daTemp doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:163]).text=[calcPaDa.daBaro doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.daBaro doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:164]).text=[calcPaDa.daDew doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.daDew doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:165]).text=[calcPaDa.da doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcPaDa.da doubleValue]]:@"";
