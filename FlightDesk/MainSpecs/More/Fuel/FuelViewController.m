...
@interface FuelViewController ()<UITextFieldDelegate>
{
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"FUE%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"FuelViewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"FUE%ld",(long)textField.tag+10000]];
...
- (IBAction)clearFuelBurn:(id)sender {
    ...
    ((UITextField *)[self.view viewWithTag:124]).text = @"";
    
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",121+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",122+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",123+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",124+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
...
- (IBAction)clearFuelNM:(id)sender {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",131+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",132+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",133+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",134+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"FUE%d",135+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
    [self.SavedRecordTableView reloadData];
}
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcFuel" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcFuel" inManagedObjectContext:context];
            CalcFuel *calcFuel=[[CalcFuel alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcFuel.name=self.nameTextField.text;
            
            calcFuel.burnTimeDec = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:121]).text doubleValue]];
            calcFuel.burnTime = ((UITextField *)[self.view viewWithTag:122]).text;
            calcFuel.burnFBhr = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:123]).text doubleValue]];
            calcFuel.burnFURequired = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:124]).text doubleValue]];
            
            calcFuel.nmGal = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:131]).text doubleValue]];
            calcFuel.nmGalTimeDec = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:132]).text doubleValue]];
            calcFuel.nmGalFBhr = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:133]).text doubleValue]];
            calcFuel.nmGalNg = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:134]).text doubleValue]];
            calcFuel.bnGalGN = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:135]).text doubleValue]];
            
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcFuel];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcFuel *calcFuelToDelete in fetchedData) {
            [context deleteObject:calcFuelToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
}
...
{
    static NSString *MyIdentifier = @"CalcFuelItem";
    ...
    CalcFuel *calcFuel=[arrayFiles objectAtIndex:indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcFuel *calcFuel=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcFuel];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcFuel*)calcFuel {
    ((UITextField *)[self.view viewWithTag:121]).text=[calcFuel.burnTimeDec doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.burnTimeDec doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:122]).text=[calcFuel.burnTime length]>0?calcFuel.burnTime:@"";
    ((UITextField *)[self.view viewWithTag:123]).text=[calcFuel.burnFBhr doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.burnFBhr doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:124]).text=[calcFuel.burnFURequired doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.burnFURequired doubleValue]]:@"";
    
    ((UITextField *)[self.view viewWithTag:131]).text=[calcFuel.nmGal doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.nmGal doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:132]).text=[calcFuel.nmGalTimeDec doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.nmGalTimeDec doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:133]).text=[calcFuel.nmGalFBhr doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.nmGalFBhr doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:134]).text=[calcFuel.nmGalNg doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.nmGalNg doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:135]).text=[calcFuel.bnGalGN doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcFuel.bnGalGN doubleValue]]:@"";
