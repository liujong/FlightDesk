...
- (void)eraserTouchesBegan{
    drawLayer.opacity = self.drawOpacity;
    [self drawLineFromForEraser:lastPoint to:lastPoint width:self.eraserDrawWidth];
}
- (void)eraserTouchesMoved{
    [self drawLineFromForEraser:lastPoint to:currentPoint width:self.eraserDrawWidth];
}
- (void)eraserTouchesEnded{
    [self commitDrawingWithOpacity:self.drawOpacity];
}
- (void) drawLineFromForEraser:(CGPoint)from to:(CGPoint)to width:(CGFloat)width
{
    UIGraphicsBeginImageContext(self.frame.size);
    [mainImage drawInRect:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    ...
    CGPathMoveToPoint(path, nil, from.x, self.frame.size.height-from.y);
    CGPathAddLineToPoint(path, nil, to.x, self.frame.size.height-to.y);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (self.toolType == DAScratchPadToolTypeEraser) {
        [self eraserTouchesBegan];
    }
