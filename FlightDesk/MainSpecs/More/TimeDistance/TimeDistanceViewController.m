...
@interface TimeDistanceViewController ()<UITextFieldDelegate>
{
...
@implementation TimeDistanceViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"TIME DISTANCE";
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                ...
                if ([[NSUserDefaults standardUserDefaults ] boolForKey:@"SavedText"]==YES) {
                    textField.text=[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"TDS%ld",(long)textField.tag+10000]];
                }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [tracker set:kGAIScreenName value:@"TimeDistanceviewController"];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UIView *aView in subView.subviews) {
            if ([aView isKindOfClass:[UITextField class]]) {
                UITextField *textField = (UITextField *)aView;
                textField.text=@"";
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%ld",(long)textField.tag+10000]];
    ...
    for (UIView *subView in self.containerView.subviews) {
        ...
        for (UITextField *textField in subView.subviews) {
            if ([textField isKindOfClass:[UITextField class]]) {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:textField.text forKey:[NSString stringWithFormat:@"TDS%ld",(long)textField.tag+10000]];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }
        }
    }
...
- (IBAction)clearDistance:(id)sender {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",101+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",102+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",103+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",104+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
...
- (IBAction)clearETA:(id)sender {
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",111+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",112+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",113+10000]];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:[NSString stringWithFormat:@"TDS%d",114+10000]];
    [[NSUserDefaults standardUserDefaults] synchronize];
    currentSelectedRow = -1;
    [self.SavedRecordTableView reloadData];
}
...
-(BOOL)exist
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CalcTimeDistance" inManagedObjectContext:managedObjectContext];
...
- (IBAction)onSave:(id)sender {
    if ([self.nameTextField.text length]>0) {
        if (![self exist]) {
            ...
            NSEntityDescription *descriptor=[NSEntityDescription entityForName:@"CalcTimeDistance" inManagedObjectContext:context];
            CalcTimeDistance *calcTimeDistance=[[CalcTimeDistance alloc] initWithEntity:descriptor insertIntoManagedObjectContext:context];
            calcTimeDistance.name=self.nameTextField.text;
            
            calcTimeDistance.timeDistanceTime = ((UITextField *)[self.view viewWithTag:101]).text;
            calcTimeDistance.timeDistanceKts = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:102]).text doubleValue]];
            calcTimeDistance.timeDistanceNM = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:103]).text doubleValue]];
            calcTimeDistance.timeDistanceSM = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:104]).text doubleValue]];
            calcTimeDistance.etaKTS = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:111]).text doubleValue]];
            calcTimeDistance.etaNM = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:112]).text doubleValue]];
            calcTimeDistance.etaSM = [NSNumber numberWithDouble:[((UITextField *)[self.view viewWithTag:113]).text doubleValue]];
            calcTimeDistance.etaTime = ((UITextField *)[self.view viewWithTag:114]).text;
            NSError *error;
            [context save:&error];
            if (!error) {
                [arrayFiles addObject:calcTimeDistance];
...
- (IBAction)onClearAll:(id)sender {
    ...
    if ([fetchedData count]>0) {
        for (CalcTimeDistance *calcTimeDistanceToDelete in fetchedData) {
            [context deleteObject:calcTimeDistanceToDelete];
        }
    }
    [self.SavedRecordTableView reloadData];
}
...
{
    static NSString *MyIdentifier = @"CalcTimeDistanceItem";
    ...
    CalcTimeDistance *calcTimeDistance=[arrayFiles objectAtIndex:indexPath.row];
    ...
    saveTxt.text = calcTimeDistance.name;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CalcTimeDistance *calcTimeDistance=[arrayFiles objectAtIndex:indexPath.row];
    [self displayDetails:calcTimeDistance];
    [self.SavedRecordTableView reloadData];
}
-(void)displayDetails:(CalcTimeDistance*)calcTimeDistance {
    ((UITextField *)[self.view viewWithTag:101]).text=[calcTimeDistance.timeDistanceTime length]>0?calcTimeDistance.timeDistanceTime:@"";
    ((UITextField *)[self.view viewWithTag:102]).text=[calcTimeDistance.timeDistanceKts doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.timeDistanceKts doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:103]).text=[calcTimeDistance.timeDistanceNM doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.timeDistanceNM doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:104]).text=[calcTimeDistance.timeDistanceSM doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.timeDistanceSM doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:111]).text=[calcTimeDistance.etaKTS doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.etaKTS doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:112]).text=[calcTimeDistance.etaNM doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.etaNM doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:113]).text=[calcTimeDistance.etaSM doubleValue]>0.0?[NSString stringWithFormat:@"%.02f", [calcTimeDistance.etaSM doubleValue]]:@"";
    ((UITextField *)[self.view viewWithTag:114]).text=[calcTimeDistance.etaTime length]>0?calcTimeDistance.etaTime:@"";
